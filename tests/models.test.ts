...
describe("Application model", () => {
  ...
  it("defaults connectionStatus to not_connected", () => {
    ...
    expect(app.connectionStatus).toBe("not_connected");
  });

  it("accepts every state in the admin-mediation connection state machine", () => {
    for (const connectionStatus of ["not_connected", "pending_admin", "contacting", "connected", "failed", "cancelled"]) {
      const err = new Application({
        tuitionPost: id(),
        tutor: id(),
        coverMessage: "msg",
  ...
  it("rejects a connectionStatus outside the known state machine", () => {
    const err = new Application({
      tuitionPost: id(),
      tutor: id(),
      coverMessage: "msg",
      expectedSalary: 5000,
      availability: "Evenings",
      connectionStatus: "chatting_on_the_phone",
    }).validateSync();
    expect(err?.errors.connectionStatus).toBeDefined();
  });
});

describe("AuditLog model", () => {
  it("requires actor, action, targetType, targetId", () => {
    const err = new AuditLog({}).validateSync();
    expect(err).not.toBeNull();
  });

  it("rejects an action outside the known catalogue", () => {
    const err = new AuditLog({
      actor: id(),
      action: "DID_SOMETHING_UNLISTED",
  ...
  it("accepts a valid audit entry", () => {
    const err = new AuditLog({
      ...
      targetType: "Application",
      targetId: id(),
      metadata: { tuitionPost: id().toString() },
    }).validateSync();
    expect(err).toBeUndefined();
  });
});
