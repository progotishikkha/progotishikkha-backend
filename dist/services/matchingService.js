...
exports.matchTutorsForPost = void 0;
 ...
 */
const scoreTutorForPost = (tutor, post) => {
    let score = 0;
    const subjectMatch = tutor.subjects.some((s) => normalize(s) === normalize(post.subject));
    const subjectPartialMatch = !subjectMatch && tutor.subjects.some((s) => normalize(post.subject).includes(normalize(s)) || normalize(s).includes(normalize(post.subject)));
    if (subjectMatch)
        score += WEIGHTS.subject;
    else if (subjectPartialMatch)
    ...
    if (post.teachingMode === "online")
        score += WEIGHTS.teachingMode;
    else if (post.teachingMode === "both")
        score += WEIGHTS.teachingMode * 0.6;
...
const matchTutorsForPost = async (post) => {
    const candidates = await tutorProfile_model_1.TutorProfile.find({
        verificationStatus: { $ne: "rejected" },
        subjects: { $exists: true, $not: { $size: 0 } },
    }).select("user subjects location availability verificationStatus isApproved");
    ...
    const alreadyNotified = await notification_model_1.Notification.find({
        type: "new_match",
        relatedId: post._id,
        recipient: { $in: scored.map((m) => m.tutor.user) },
    }).distinct("recipient");
    const alreadyNotifiedSet = new Set(alreadyNotified.map((id) => id.toString()));
    const toNotify = scored.filter((m) => !alreadyNotifiedSet.has(m.tutor.user.toString()));
    await Promise.all(toNotify.map((m) => (0, notification_service_1.notify)({
...
exports.matchTutorsForPost = matchTutorsForPost;
